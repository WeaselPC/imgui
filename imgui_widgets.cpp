...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        if ((flags & ImGuiTextFlags_CodeBlock) != 0)
            ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ColorConvertFloat4ToU32({ 0.13f, 0.14f, 0.17f, 1.0f }));
...
void ImGui::CodeV(const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const char* text, * text_end;
    ImFormatStringToTempBufferV(&text, &text_end, fmt, args);
    TextEx(text, text_end, ImGuiTextFlags_CodeBlock | ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
void ImGui::CodeWrapped(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImDrawFlags drawFlags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding, (ImDrawFlags_)drawFlags);
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, ImDrawFlags drawFlags)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, drawFlags);
}
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis, bool mirror)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        if (mirror) {
            return ImRect(outer_rect.Min.x, inner_rect.Min.y, outer_rect.Min.x + scrollbar_size, inner_rect.Max.y - border_size);
        }
        else {
...
void ImGui::Scrollbar(ImGuiAxis axis, bool mirror)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    ImRect bb = GetWindowScrollbarRect(window, axis, window->MirrorScrollbar);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const ImVec2 padding(style.FrameBorderSize, style.FrameBorderSize);
    ...
    if (style.FrameBorderSize > 0.0f)
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), 0.0f, ImDrawFlags_None, style.FrameBorderSize);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags, ImDrawFlags drawFlags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding), (ImDrawFlags_)drawFlags);
    // Render
    if (bg_col.w > 0.0f)
    {
...
bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, ImDrawFlags drawFlags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(window->GetID(str_id), user_texture_id, image_size, uv0, uv1, ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1), ImGuiButtonFlags_None, drawFlags);
}
...
bool ImGui::DrawDragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImDrawFlags drawFlags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, style.FrameRounding, (ImDrawFlags_)drawFlags);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        value_changed |= DrawDragScalar("", data_type, p_data, v_speed, p_min, p_max, format, flags);
        PopID();
        PopItemWidth();
...
{
    return DrawDragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags, drawFlags);
}
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DrawDragScalar("##min", ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DrawDragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragInt(const char* label, int* v, float v_speed, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, ImDrawFlags drawFlags)
{
    return DrawDragScalar(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max, format, flags, drawFlags);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const bool rightSideArrow = (flags & ImGuiTreeNodeFlags_ArrowOnRightSide) != 0;
    if (!label_end)
        label_end = FindRenderedTextEnd(label);
    const ImVec2 label_size = CalcTextSize(label, label_end, false);

    const float text_offset_x = (rightSideArrow ? 0 : g.FontSize) + (display_frame ? padding.x * 3 : padding.x * 2);   // Collapsing arrow width + Spacing
    ...
    if (rightSideArrow) {
        interact_bb.Min.x = frame_bb.Max.x - (text_width + (label_size.x > 0.0f ? style.ItemSpacing.x * 2.0f : 0.0f));
        interact_bb.Max.x = frame_bb.Max.x;
    }
    ...
    bool is_mouse_x_over_arrow = false;
    if (rightSideArrow) {
        const float arrow_hit_x1 = (frame_bb.Max.x - (g.FontSize + padding.x)) - style.TouchExtraPadding.x;
        const float arrow_hit_x2 = (frame_bb.Max.x - (g.FontSize + padding.x)) + (g.FontSize + padding.x * 2.0f) + style.TouchExtraPadding.x;
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            float xPos = rightSideArrow ? frame_bb.Max.x - (g.FontSize + padding.x) : text_pos.x - text_offset_x + padding.x;
            RenderArrow(window->DrawList, ImVec2(xPos, text_pos.y), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 1.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            float xPos = rightSideArrow ? frame_bb.Max.x - (g.FontSize + padding.x) : text_pos.x - text_offset_x + padding.x;
            RenderArrow(window->DrawList, ImVec2(xPos, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
        }
        if (g.LogEnabled)
            LogSetNextTextDecoration(">", NULL);
    }
